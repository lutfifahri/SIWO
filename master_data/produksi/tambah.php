...
if (isset($_POST['submit'])) {
    ...
    $query = "INSERT INTO tb_produksi (nama, harga, jenis, detail) VALUES ('$nama', '$harga', '$jenis', '$detail')";
    if ($conn->query($query)) {
        ...
        for ($i = 0; $i < count($_FILES['gambar']['name']); $i++) {
            ...
            $query = "INSERT INTO tb_gambar_produksi (id_produksi, gambar) VALUES ($last_id, '$target_file')";
            $conn->query($query);
        }
        echo "<script>alert('Data produksi berhasil ditambahkan!');</script>";
