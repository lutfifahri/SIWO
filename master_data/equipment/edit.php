<?php
if (isset($_GET['id'])) {
    $query = "SELECT * FROM tb_equipment WHERE id=" . $_GET['id'];
    ...
    $query = "SELECT * FROM tb_gambar_equipment WHERE  id_equipment=" . $_GET['id'];
...
if (isset($_POST['submit'])) {
    ...
    $query = "UPDATE tb_equipment SET nama='$nama', harga='$harga', jenis='$jenis', detail='$detail' WHERE id=" . $_GET['id'];
    if ($conn->query($query)) {
        if ($_FILES['gambar']['error'][0] != 4) {
            if ($conn->query("DELETE FROM tb_gambar_equipment WHERE id_equipment=" . $_GET['id'])) {
                $target_dir = "uploads/";
                for ($i = 0; $i < count($_FILES['gambar']['name']); $i++) {
                    ...
                    $query = "INSERT INTO tb_gambar_equipment (id_equipment, gambar) VALUES (" . $_GET['id'] . ", '$target_file')";
                    $conn->query($query);
                }
            }
        }
        echo "<script>alert('Data equipment berhasil diperbaharui!');</script>";
                            ...
                            <div class="card-body">
                                ...
                                <div class="form-group">
                                    ...
                                    <select name="jenis" id="jenis" class="form-control">
                                        ...
                                        <option <?= $row['jenis'] === "LIGHTING" ? "selected" : ''; ?> value="LIGHTING">Lighting</option>
                                        <option <?= $row['jenis'] === "SOUND" ? "selected" : ''; ?> value="SOUND">Sound</option>
                                    </select>
                                </div>
                                <div class="form-group">
